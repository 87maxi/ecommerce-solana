...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      "[MINT-AFTER-PAYMENT] Processing mint for payment:",
      paymentIntentId,
    );
    ...
    if (paymentIntent.status !== "succeeded") {
      console.error(
        "[MINT-AFTER-PAYMENT] Payment not succeeded:",
    ...
    if (!order) {
      ...
      console.log(
        "[MINT-AFTER-PAYMENT] Order not found by ID, searching by invoice/wallet...",
      );
    ...
    try {
      ...
      console.log(
        "[MINT-AFTER-PAYMENT] Tokens minted successfully:",
        mintResult,
      );
