...
function SuccessPageContent() {
  ...
  const fetchUpdatedBalance = async (address: string) => {
    ...
    try {
      ...
      // Retry to allow blockchain state update
      for (let i = 0; i < 5; i++) {
        try {
  ...
  useEffect(() => {
    const verifyPurchase = async () => {
      ...
      const storedAmount = sessionStorage.getItem("amount") || "0";
      ...
      try {
        ...
        const response = await fetch(`${pasarelaUrl}/api/verify-minting?payment_intent=${paymentIntentId}`);
        ...
        if (data.success) {
          ...
          setTimeout(verifyPurchase, 5000);
        }
      } catch (err) {
  ...
  const handleReturnToStore = () => {
    const redirectUrl = sessionStorage.getItem("redirect_url") || "http://localhost:3030";
    window.location.href = `${redirectUrl}?success=true&amount=${payment.amount}&invoice=${payment.invoice}&wallet=${payment.wallet}`;
  };
        ...
        <div className="text-center space-y-6">
          {payment.status === "loading" || payment.status === "processing" ? (
            <>
              ...
              <h1 className="text-2xl font-bold text-white">Verificando Transacción</h1>
            ...
            <>
              ...
              <div className="bg-slate-800/50 rounded-2xl p-5 border border-slate-700/50 text-left space-y-3">
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="text-emerald-400 font-bold">{payment.amount} EURT</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                  <span className="text-slate-400">Factura</span>
                  <span className="text-white font-mono">{payment.invoice}</span>
                </div>
                <div className="pt-3 border-t border-slate-700 flex justify-between items-end">
                  <div>
                    <p className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Nuevo Balance</p>
                    <div className="flex items-baseline gap-1">
                      <span className="text-xl font-bold text-white">{updatedBalance || "..."}</span>
                      <span className="text-xs text-slate-400">EURT</span>
                    </div>
                  </div>
                  {payment.txHash && (
                    <a
                      href={`https://explorer.solana.com/tx/${payment.txHash}?cluster=custom&customUrl=http://localhost:8899`}
                      target="_blank"
            ...
            <>
              ...
              <h1 className="text-2xl font-bold text-white">Hubo un Problema</h1>
              <p className="text-red-400 text-sm">
                {payment.error || "No pudimos verificar la transacción automáticamente."}
              </p>
              <button
                onClick={() => router.push("/")}
