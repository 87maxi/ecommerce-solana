...
fn main() -> Result<()> {
    ...
    for (i, step) in fixture.steps.iter().enumerate() {
        println!("\n🔹 [{}/{}] {}", i + 1, fixture.steps.len(), step.description);

        match step.instruction.as_str() {
            "register_company" => {
                let name = step.args[0].as_str().ok_or(anyhow!("Invalid name"))?;
                let description = step.args[1].as_str().ok_or(anyhow!("Invalid description"))?;

                // Read current next_company_id from chain
                let global_data: solana_ecommerce::state::GlobalState = program.account(global_pda)?;
            ...
            "add_product" => {
                let name = step.args[0].as_str().ok_or(anyhow!("Invalid name"))?;
                let price = step.args[1].as_u64().ok_or(anyhow!("Invalid price"))?;
                let stock = step.args[2].as_u64().ok_or(anyhow!("Invalid stock"))?;

                let company_pda = *context.get("@company_pda").ok_or(anyhow!("Company PDA not set — run register_company first"))?;
                ...
                let company_data: solana_ecommerce::state::Company = program.account(company_pda)?;
